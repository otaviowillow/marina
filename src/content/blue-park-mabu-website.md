---
path: "/blue-park-mabu-website"
date: "2018-01-02"
title: "Blue Park Mabu Website"
client: "Mabu Thermas Grand Resort"
role: "UX/UI Designer"
homeImage: "/img/home-mabu.png"
mainImage: "/img/mabu-main.png"
feat: false
summary: "Foz do Iguaçu, in Paraná, is one of the first most visited tourist destinations in Brazil. It has several tourist attractions such as the Iguaçu Falls, the Itaipu Hydroelectric Power Plant including the Triple Border, between Brazil, Paraguay and Argentina. Foz now also has the newest water park in southern Brazil."
responsabilities: "Domain Research, Mind map, Sitemap, usability testing, style guide, mid and high fidelity prototypes."
portfolio: [ "/img/mabu-portfolio-1.png", "/img/mabu-portfolio-2.png", "/img/mabu-portfolio-3.png" ]
---

### About
   
Foz do Iguaçu, in Paraná, is one of the first most visited tourist destinations in Brazil. It has several tourist attractions such as the Iguaçu Falls, the Itaipu Hydroelectric Power Plant including the Triple Border, between Brazil, Paraguay and Argentina. Foz now also has the newest water park in southern Brazil. 
The park has the third-largest thermal beach with waves in the world and promises a lot of fun for everyone. There are 9 types of waves that reach 1.20m in height, with natural sand and a paradisiacal view.

### Domain Research

I started the project looking for the competitors and doing the benchmarking of those websites, analyzing their responsivity, social media accounts, performance, and overall numbers.

![enter image description here](/img/aquatica.png)
![enter image description here](/img/beach-park.png)
![enter image description here](/img/eco-parque.png)

### Planning

In this phase, I started to plan the project with a mind map to visually organize the information. With all these information I created the sitemap.

![enter image description here](/img/mabu-mindflow.png)
![enter image description here](/img/mabu-map.png)

### Design

Finally, after collecting all the necessary information, I started to create the wireframes.

![enter image description here](/img/mabu-ux-home.png.png)
![enter image description here](/img/mabu-ux-home-mobile.png.png)

### Style Guide

![enter image description here](/img/aquatica.png)

### UI

With everything defined I initiated the user interface process. The first step was creating a mood board to actually conceive the project elements together, get visual references and insights from this before actually design the screens.